...
public class Md5Utils {
    ...
    private static byte[] md5sum(byte[] data) {
        try {
            MessageDigest mdTemp = MessageDigest.getInstance(ENCRYPT_ALGORITHM);
            mdTemp.update(data);
    ...
    private static byte[] md5sum(File file) {
        ...
        try {
            ...
            md5 = MessageDigest.getInstance(ENCRYPT_ALGORITHM);
    ...
    public static String getMD5(File file) {
        if (file != null && file.exists()) return new String(convertToHexString(md5sum(file)));
        else return null;
    }
}